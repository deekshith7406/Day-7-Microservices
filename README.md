# Day 5–7 — Service-to-Service Communication, Discovery, Gateway & Config
...
Days 6 and 7 each explicitly say to upgrade "the Day 5/6 system," so both
extend this project rather than starting yet another one.

(This folder used to be named `day5-service-communication`. It was renamed
to `day5-7-service-platform` once Day 6 made "day5" in the name
misleading — the Java package names underneath still say `day5` for the
same historical reason; a full rename would touch every file for no
functional benefit, so only the visible project name changed.)
...
```
day5-7-service-platform/
├── config-repo/        → git repo of centralized config (Day 7)
├── config-server/         → serves config-repo/ over HTTP, port 8888
...
**Start order matters.** User Service and Order Service now depend on
BOTH the discovery server (Day 6) AND the config server (Day 7) being up
first — Config Server has no dependencies of its own, so it's safe to
start anytime, but starting it before the two application services avoids
the "critical config missing" startup failure discussed in the Day 7
section below (unless you're deliberately triggering that as an exercise).
